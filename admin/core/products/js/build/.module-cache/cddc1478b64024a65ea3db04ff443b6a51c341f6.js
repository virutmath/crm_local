...
ImportScript.productList = ImportScript.productListData.map(function (item) {
    var list = [];
    list.push(new ProductItem(item));
    return list;
});
...
ImportScript.addProduct = function (pro_id) {
    if(!this.issetProduct(this.importList,pro_id)) {
        this.importList.push(this.getProductFromList(pro_id));
        this.init();
    }
    //active record
...
ImportScript.getProductFromList = function (pro_id) {
    for(var i in this.productList){
        if(this.productList[i].pro_id == pro_id){
            return this.productList[i].getInstances();
        }
    }
    return false;
};
ImportScript.getProductFromImport = function (pro_id) {
    for(var i in this.importList){
        if(this.importList[i].pro_id == pro_id){
            return this.importList[i].getInstances();
        }
    }
    return false;
};
