...
ImportScript.react.renderTableImport = function () {
    ...
    if(ImportScript.billInfo.debit) {
        ...
        React.render(React.createElement(FormattedNumber, {value: ImportScript.billInfo.totalMoney - ImportScript.billInfo.debitMoney}), $('#money-debit')[0]);
    }
...
ImportScript.removeProduct = function (pro_id) {
    ...
    if (this.importList.length) {
        if (this.importList.length > indexOf) {
            this.activeProductImport(this.importList[indexOf].pro_id);
        } else if(this.importList.length == indexOf){
            this.activeProductImport(this.importList[this.importList.length].pro_id)
        }else {
            this.activeProductImport(this.importList[this.importList.length - 1].pro_id)
        }
    }
