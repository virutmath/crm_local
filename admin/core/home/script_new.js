...
HomeScript.keyUpFunction = function () {
            ...
            HomeScript.currentDesk.menuList.map(function (menuItem, index) {
                ...
                return (
                    React.createElement("tbody", null,
                        React.createElement("tr", {id: rowData.id, className: "menu-desk-menu record-item", onClick: onClickFn},
                            React.createElement("td", {className: "center"}, index + 1),
                            React.createElement("td", null, rowData.name),
                            React.createElement("td", {className: "center"}, rowData.unit),
                            React.createElement("td", {className: "center"}, rowData.number),
                            React.createElement("td", {className: "text-right"}, rowData.price),
                            React.createElement("td", {className: "center"}, rowData.discount),
                            React.createElement("td", {className: "text-right"}, rowData.total)
                        )
                    )
                )
            }),
...
HomeScript.view.fillDataToInput = function () {
    ...
    domElement.customerCash.autoNumeric('set', 0);
...
HomeScript.view.printBills = function () {
    if (HomeScript.currentDesk.deskItem.des_id && HomeScript.currentMenu.menuItem.men_id) {
        // lấy id bàn và menu để in hóa đơn
        var desk_id = HomeScript.currentDesk.deskItem.des_id;
        var extra_fee = HomeScript.currentDesk.billInfo.extraFee;
        var discount = HomeScript.currentDesk.billInfo.customerDiscount;
        var vat_value = HomeScript.currentDesk.billInfo.VAT;
        var customer_cash = HomeScript.domElement.customerCash.autoNumeric('get');
        //thời gian
        var time = HomeScript.currentDesk.billInfo.startTimeStr;
        //id va tên khách hàng
        var customer_id = HomeScript.currentDesk.billInfo.customerID;
        var customer_name = HomeScript.currentDesk.billInfo.customerName;
        //tên nhân viên
        var staff_name = HomeScript.currentDesk.billInfo.staffName;
        // tổng tiền
        var total = HomeScript.currentDesk.billInfo.totalMoney;
        var mwindow = new Mindows();
        mwindow.width = 600;
        mwindow.height = 600;
        mwindow.resize = true;
...
HomeScript.view.resetInput = function () {
    ...
    if (!HomeScript.currentDesk.domElement === null) {
