...
var TableRow = React.createClass({displayName: "TableRow",
    ...
    render : function () {
        ...
        return React.createElement("tbody", null, 
                React.createElement("tr", {id: element_id, className: "menu-desk-menu record-item", onClick: onClickFn}, 
                    ...
                    React.createElement("td", {className: "center"}, React.createElement(FormattedNumber, {value: this.props.discount, style: "percent"})), 
...
HomeScript.deleteMenu = function(elem) {
    var menu_id = HomeScript.currentMenu.menuItem.men_id;
    if(!menu_id){
        alert('Chọn thực đơn để xóa');
    }
    $.ajax({
        type : 'post',
        url : 'ajax.php',
        data : {action : 'deleteMenu',menu_id:menu_id},
        dataType : 'json',
        success: function(resp){
            if(resp.success == 1){
                loadingProgress('show');
...
HomeScript.view.settingMenuẼ = function () {
    var result_percent = 0, result_money = 0;
    //nếu current_menu không tồn tại thì return luôn
    if (!HomeScript.currentMenu.menuItem.men_id) {
        return false;
        ...
        items: {
            active : {
                ...
                callback: function(key, opt) {
                    var _this = $(this);
                    if (confirm('Bạn muốn hủy thực đơn này?')) {
                        HomeScript.deleteMenu(_this);
                        //_this.remove();
                    }
                }
            }
